...
- **Implicit Handlers**: Create handlers directly from methods for quick prototyping.
- **Support for MTProto**: Full support for MTProto via `TelegratorWClient` (Wider Bot).
- **Extensible State Management**: Built-in mechanisms for user/chat state with support for In-Memory and Redis storage.
